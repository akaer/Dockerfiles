...
'
        ...
        -- Nacharbeiten für OneIM Dbs

        set @SQL = 'if exists (select top 1 1 from ' + @DatabaseName + '.sys.tables where name = ''DialogDatabase'')
        begin
            alter database [' + @DatabaseName + '] SET PARAMETERIZATION FORCED WITH NO_WAIT -- #31843
            alter database [' + @DatabaseName + '] set NEW_BROKER
            alter database [' + @DatabaseName + '] set enable_broker
            exec ' + @DatabaseName + '.dbo.QBM_PDBQueuePrepare 0,1
            exec ' + @DatabaseName + '.dbo.QBM_PWatchDogPrepare
        end
        '
